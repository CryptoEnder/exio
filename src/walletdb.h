...
public:
    ...
    bool WritezEXOSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadzEXOSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadzEXOSeed_deprecated(uint256& seed);
    bool ErasezEXOSeed();
    bool ErasezEXOSeed_deprecated();

    bool WritezEXOCount(const uint32_t& nCount);
    bool ReadzEXOCount(uint32_t& nCount);
