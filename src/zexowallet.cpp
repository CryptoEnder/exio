...
CzEXOWallet::CzEXOWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zexo seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadzEXOSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.ErasezEXOSeed_deprecated()) {
                    LogPrintf("%s: Updated zEXO seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zexo seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zexo wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzEXOWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WritezEXOCount(nCountLastUsed);
    else if (!walletdb.ReadzEXOCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzEXOWallet::Lock()
{
    seedMaster = 0;
}

void CzEXOWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzEXOWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedTozEXO(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzEXOWallet::LoadMintPoolFromDB()
{
...
void CzEXOWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzEXOWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzEXOWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zexoTracker->HasPubcoinHash(pMint.first)) {
...
bool CzEXOWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedTozEXO(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zexoTracker which also adds to database
    pwalletMain->zexoTracker->Add(dMint, true);
...
void CzEXOWallet::SeedTozEXO(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzEXOWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzEXOWallet::GenerateDeterministiczEXO(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzEXOWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzEXOWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
