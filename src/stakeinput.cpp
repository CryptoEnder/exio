...
// The zEXO block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zEXO is a hash of the serial
...
{
    //Create an output returning the zEXO that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreatezEXOOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zEXO output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zEXO", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreatezEXOOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zEXO output", __func__);
...
{
    CzEXOTracker* zexoTracker = pwallet->zexoTracker.get();
    CMintMeta meta;
    if (!zexoTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zexoTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a EXO stake is the outpoint
