...
#ifndef EXO_zEXOWALLET_H
#define EXO_zEXOWALLET_H
...
public:
    CzEXOWallet(std::string strWalletFile);
    ...
    void GenerateDeterministiczEXO(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedTozEXO(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //EXO_zEXOWALLET_H
